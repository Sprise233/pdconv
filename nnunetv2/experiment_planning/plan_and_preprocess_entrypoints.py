...
def plan_experiment_entry():
    ...
    parser.add_argument('-overwrite_plans_name', default=None, required=False,
                        help='[OPTIONAL] DANGER ZONE! If you used -gpu_memory_target, -preprocessor_name or '
                             ...
                             'differently named plans file such that the nnunetv1 default plans are not '
                             'overwritten. You will then need to specify your custom plans file with -p whenever '
                             'running other nnunetv1 commands (training, inference etc)')
    args, unrecognized_args = parser.parse_known_args()
...
if __name__ == '__main__':
    ...
    c = ['3d_fullres']
    default_np = {"2d": 8, "3d_fullres": 4, "3d_lowres": 8}
    np = [default_np[i] if i in default_np.keys() else 4 for i in c]
    extract_fingerprints(dataset_ids=dataset_ids)
    # {ExperimentPlanner, LgNetPlanner, ResEncUNetPlanner}
    plans_identifier = plan_experiments(dataset_ids=dataset_ids, gpu_memory_target_in_gb=8, experiment_planner_class_name='LgNetPlanner')
    preprocess(dataset_ids, plans_identifier, c, np, False)
    # preprocess(dataset_ids, '/home/cuiqi/model/nnUnet/data/processed/Dataset002_ACDC/LgNetPlans', c, np, False)
