...
def create_ACDC_split(labelsTr_folder: str, seed: int = 1234):
...
def copy_files(src_data_folder: Path, train_dir: Path, labels_dir: Path, test_dir: Path, out_test_labels_dir: Path, test_list = None):
    ...
    for patient_dir in patients_train:
        for file in patient_dir.iterdir():
            if test_list is not None:
                label_name = file.name.split('.')[0].replace('_gt', "") + '.nii.gz'
                if file.suffix == ".gz" and "_gt" not in file.name and "_4d" not in file.name and file.name not in test_list:
                ...
                elif file.suffix == ".gz" and "_gt" in file.name and label_name not in test_list:
                    shutil.copy(file, labels_dir / file.name.replace("_gt", ""))
                elif file.suffix == ".gz" and "_gt" not in file.name and "_4d" not in file.name and file.name in test_list:
                    shutil.copy(file, test_dir / f"{file.stem.split('.')[0]}_0000.nii.gz")
                elif file.suffix == ".gz" and "_gt" in file.name and label_name in test_list:
                    print(file.name)
                    shutil.copy(file, out_test_labels_dir / file.name.replace("_gt", ""))
            else:
    ...
    if test_list is None:
        ...
        for patient_dir in patients_test:
            for file in patient_dir.iterdir():
                if file.suffix == ".gz" and "_gt" in file.name:
                    shutil.copy(file, out_test_labels_dir / file.name.replace("_gt", ""))
    return num_training_cases


def convert_acdc(src_data_folder: str, dataset_id=27, test_list=None):
    out_dir, train_dir, labels_dir, test_dir, out_test_labels_dir = make_out_dirs(dataset_id=dataset_id)
    num_training_cases = copy_files(Path(src_data_folder), train_dir, labels_dir, test_dir, out_test_labels_dir, test_list=test_list)
...
#         "-i",
#         "--input_folder",
#         default='/home/cuiqi/model/nnUnet/data/ACDC/database',
#         type=str,
#         help="The downloaded ACDC dataset dir. Should contain extracted 'training' and 'testing' folders.",
#     )
#     parser.add_argument(
#         "-d", "--dataset_id", required=False, type=int, default=2, help="nnU-Net Dataset ID, default: 27"
#     )
...
#     save_json(split, join(preprocessed_folder, 'splits_final.json.json'), sort_keys=False)
#
#     print("Done!")
