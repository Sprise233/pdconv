...
"""
PLEASE READ paths.md FOR 
INFORMATION TO HOW TO SET THIS UP
"""
...
if None in (nnUNet_raw, nnUNet_preprocessed, nnUNet_results):
    try:
        print('使用config数据和保存路径')
        current_path = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.abspath(os.path.join(current_path, '../'))
        config_path = os.path.join(project_root, 'config.yaml')
        config = load_yaml(config_path)
        nnUNet_raw = config['environment']['nnUNet_raw']
        nnUNet_preprocessed = config['environment']['nnUNet_preprocessed']
        nnUNet_results = config['environment']['nnUNet_results']
    except:
        print('根目录中没有config.yaml文件')
