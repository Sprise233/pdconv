...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'), config: dict = None):
        ...
        if self.is_ddp:  # implicitly it's clear that we use cuda in this case
            ...
            self.device = torch.device(type='cuda', index=config['training']['gpu_index'][self.local_rank])
        else:
            if self.device.type == 'cuda':
                # we might want to let the user pick this but for now please pick the correct GPU with CUDA_VISIBLE_DEVICES=X
                torch.cuda.set_device(0 if config is None else config['training']['gpu_index'][0])
            self.device = torch.device(type='cuda', index=int(0 if config is None else config['training']['gpu_index'][0]))
            print(f"Using device: {self.device}")

        # dataloader and dataset
        self.dataloader_2D = nnUNetDataLoader2D
        self.dataloader_3D = nnUNetDataLoader3D
        self.dataset = nnUNetDataset
        ...
        # unlike the previous nnunetv1 folder_with_segs_from_previous_stage is now part of the plans. For now it has to
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            if torch.__version__.startswith('2.') and self._do_i_compile():
    ...
    def do_split(self):
        ...
        so always the same) and save it as splits_final.json.json file in the preprocessed data directory.
        Sometimes you may want to create your own split for various reasons. For this you will need to create your own
        splits_final.json.json file. If this file is present, nnU-Net is going to use it and whatever splits are defined in
        ...
        if self.fold == "all":
            ...
            splits_file = join(self.preprocessed_dataset_folder_base, "splits_final.json.json")
            dataset = self.dataset(self.preprocessed_dataset_folder, case_identifiers=None,
    ...
    def get_tr_and_val_datasets(self):
        ...
        dataset_tr = self.dataset(self.preprocessed_dataset_folder, tr_keys,
                                   folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage,
                                   num_images_properties_loading_threshold=0)
    ...
    def get_dataloaders(self):
        ...
        if dim == 2:
            dl_tr = self.dataloader_2D(dataset_tr, self.batch_size,
                                       initial_patch_size,
                                       self.configuration_manager.patch_size,
                                       self.label_manager,
                                       oversample_foreground_percent=self.oversample_foreground_percent,
                                       sampling_probabilities=None, pad_sides=None, transforms=tr_transforms)
            dl_val = self.dataloader_2D(dataset_val, self.batch_size,
                                        self.configuration_manager.patch_size,
                                        self.configuration_manager.patch_size,
                                        self.label_manager,
                                        oversample_foreground_percent=self.oversample_foreground_percent,
            ...
            dl_tr = self.dataloader_3D(dataset_tr, self.batch_size,
                                       initial_patch_size,
                                       self.configuration_manager.patch_size,
                                       self.label_manager,
                                       oversample_foreground_percent=self.oversample_foreground_percent,
                                       sampling_probabilities=None, pad_sides=None, transforms=tr_transforms)
            dl_val = self.dataloader_3D(dataset_val, self.batch_size,
                                        self.configuration_manager.patch_size,
                                        self.configuration_manager.patch_size,
                                        self.label_manager,
                                        oversample_foreground_percent=self.oversample_foreground_percent,
