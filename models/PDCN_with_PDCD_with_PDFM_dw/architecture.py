...
from models.PDCN_with_PDCD_with_PDFM_dw.blocks.decoder import Decoder
...
class PDCN_with_PDCD_with_PDFM_dw(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
...
if __name__ == '__main__':
    ...
    data_shape = (1, 1, 16, 160, 160)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(device)
    data = torch.randn(data_shape, device=device)
    model = get_network_from_plans(arch_class_name='models.PDCN_with_PDCD_with_PDFM_dw.architecture.PDCN_with_PDCD_with_PDFM_dw',
