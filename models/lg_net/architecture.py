...
class LGUnet(nn.Module):
    def __init__(self,
        ...
        self.encoder = LGEncoder(input_channels, n_stages, features_per_stage, strides,
                               n_blocks_per_stage, conv_bias, norm_op, norm_op_kwargs, dropout_op,
                               dropout_op_kwargs, nonlin, nonlin_kwargs, return_skips=True,
                               nonlin_first=nonlin_first)

        self.decoder = LGDecoder(self.encoder, num_classes, n_conv_per_stage_decoder, deep_supervision, nonlin_first=nonlin_first)
...
if __name__ == '__main__':
    ...
    data_shape = (2, 1, 32, 32, 32)
    data = torch.randn(data_shape)
    model = get_network_from_plans(arch_class_name='models.lg_net.architecture.LGUnet',
                                   arch_kwargs={'n_stages': 6, 'features_per_stage': [32, 64, 128, 256, 320, 320],
                                                'strides': [[1, 1, 1], [1, 2, 2], [2, 2, 2], [2, 2, 2], [1, 2, 2],
                                                            [1, 2, 2]], 'n_blocks_per_stage': [1, 3, 4, 6, 6, 6],
