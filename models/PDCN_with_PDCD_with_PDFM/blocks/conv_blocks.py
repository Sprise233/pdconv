...
from models.utils.BasicConv import BasicConv3D

# from models.utils.MedNeXt import MedNeXt_D as PDConv
from models.utils.PDConv import PDConv_A as PDConv

from models.utils.down_sampling import haar_downsample_3d_efficient, HaarDownSampling3D
...
class DownSampling(nn.Module):
    def __init__(self,
        ...
        self.conv_block_pd1 = PDConv(input_channels, output_channels, initial_stride, conv_bias, norm_op,
                                    norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs)

        self.conv_block_pd2 = PDConv(output_channels, output_channels, 1, conv_bias, norm_op,
                                    norm_op_kwargs, dropout_op, dropout_op_kwargs, None, None)
    ...
    def forward(self, x):
        pd_feature = self.conv_block_pd2(self.conv_block_pd1(x))
    ...
    def compute_conv_feature_map_size(self, input_size):
        ...
        output += self.conv_block_pd1.compute_conv_feature_map_size(input_size)
        output += self.conv_block_pd2.compute_conv_feature_map_size(input_size)
        return output
...
#         )
#
#         self.pdconv1 = PDConv(input_channels, input_channels // 4, 1, conv_bias, norm_op,
#                                norm_op_kwargs, None, None, nonlin, nonlin_kwargs)
#         self.pdconv2 = PDConv(input_channels // 4, input_channels, 1, conv_bias, norm_op,
#                               norm_op_kwargs, None, None, None, None)
#
#     def forward(self, x1, x2):
#         att = torch.sigmoid(self.pdconv2(self.pdconv1(x1 + x2)))
#         return x1 * att + x2 * (1 - att)
#
#
#     def compute_conv_feature_map_size(self, input_size):
#         output = 0
#         output += self.pdconv1.compute_conv_feature_map_size(input_size)
#         output += self.pdconv2.compute_conv_feature_map_size(input_size)
#         return output
