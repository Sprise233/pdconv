...
class PositionalAttentionModule(nn.Module):
    def __init__(self):
        ...
        self.conv=nn.Conv3d(in_channels=2,out_channels=1,kernel_size=(7,7,7),padding=3)
    def forward(self,x):
        max_x=custom_max(x,dim=(1,4),keepdim=True)
        avg_x=torch.mean(x,dim=(1,4),keepdim=True)
        att=torch.cat((max_x,avg_x),dim=4)
        att = att.permute((0, 4, 2, 3, 1))
        att=self.conv(att)
...
class SemanticAttentionModule(nn.Module):
    ...
    def forward(self,x):
        max_x=custom_max(x,dim=(1,2,3), keepdim=False)
        avg_x=torch.mean(x,dim=(1,2,3), keepdim=False)
        max_x=self.linear(max_x)
        avg_x=self.linear(avg_x)
        att=max_x+avg_x
        att=torch.sigmoid(att).unsqueeze(-2).unsqueeze(-2).unsqueeze(-2)
        return x*att
...
class SliceAttentionModule(nn.Module):
    ...
    def forward(self,x):
        ...
        max_x=custom_max(x,dim=(2,3,4),keepdim=False)
        avg_x=torch.mean(x,dim=(2,3,4),keepdim=False)
        max_x=self.linear(max_x)
        avg_x=self.linear(avg_x)
        att=max_x+avg_x
        # if self.uncertainty:
        ...
        #     factor=factor.view(1, temp.shape[0], -1,self.rank)
