...
def get_matching_conv(conv_op: Type[_ConvNd] = None, dimension: int = None) -> Type[_ConvNd]:
    """
    You MUST set EITHER conv_op OR dimension. Do not set both!
...
def is_trans_conv(conv_op):
    if conv_op == nn.ConvTranspose1d or conv_op == nn.ConvTranspose2d or conv_op == nn.ConvTranspose3d:
        return True
    return False
...
def maybe_convert_scalar_to_list(conv_op, scalar):
    ...
    if not isinstance(scalar, (tuple, list, np.ndarray)):
        if conv_op == nn.Conv2d or conv_op == nn.ConvTranspose2d:
            return [scalar] * 2
        elif conv_op == nn.Conv3d or conv_op == nn.ConvTranspose3d:
            return [scalar] * 3
        elif conv_op == nn.Conv1d or conv_op == nn.ConvTranspose1d:
            return [scalar] * 1
        else:
...
def avg_pool(x, dim, keepdim=True):
    temp_x = x
    for i in dim:
        temp_x = torch.mean(temp_x, dim=i, keepdim=True)
    if not keepdim:
        temp_x = temp_x.squeeze()
    return temp_x

def max_pool(x,dim,keepdim=True):
    temp_x=x
    for i in dim:
